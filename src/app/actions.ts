...
export async function createPaymentAction(formData: FormData) {
  await requireEdit();
  const { createPayment } = await import("@/modules/ledger/service");
  ...
  const created = await createPayment({
    ...
    comment: String(formData.get("comment") ?? ""),
    workId: String(formData.get("workId") ?? "") || null,
    counterpartyId: String(formData.get("counterpartyId") ?? "") || null,
  });
  await logMutation("create", "Оплаты", created.id, `${type}: ${amount}`);
...
export async function updatePaymentAction(formData: FormData) {
  await requireEdit();
  const { updatePayment } = await import("@/modules/ledger/service");
  const id = String(formData.get("id") ?? "");
  const date = String(formData.get("date") ?? "");
  const amount = parseMoney(formData.get("amount"));
  const type = String(formData.get("type") ?? "expense") as "income" | "expense";
  ...
  await logMutation("update", "Оплаты", id, `${type}: ${amount}`);
  revalidatePath("/payments");
  revalidatePath("/works");
  revalidatePath("/");
  revalidatePath("/history");
...
export async function deletePaymentAction(formData: FormData) {
  await requireEdit();
  const { deletePayment } = await import("@/modules/ledger/service");
  const id = String(formData.get("id") ?? "");
  await deletePayment(id);
  await logMutation("delete", "Оплаты", id, id);
  revalidatePath("/payments");
  revalidatePath("/works");
  revalidatePath("/");
  revalidatePath("/history");
...
function parseProductionForm(formData: FormData) {
  const received = String(formData.get("receivedAt") ?? "");
  ...
  return {
    ...
    ke: String(formData.get("ke") ?? ""),
    status: String(formData.get("status") ?? "в работе"),
    income: parseMoney(formData.get("income") || "0"),
...
export async function createProductionOrderAction(formData: FormData) {
  await requireEdit();
  const { createProductionOrder } = await import("@/modules/production/service");
  const data = parseProductionForm(formData);
  const created = await createProductionOrder(data);
  await logMutation(
    "create",
    "Продакшн",
    created.id,
    data.orderCode || data.project || created.id,
  );
...
export async function updateProductionOrderAction(formData: FormData) {
  await requireEdit();
  const { updateProductionOrder } = await import("@/modules/production/service");
  const id = String(formData.get("id") ?? "");
  const data = parseProductionForm(formData);
  await updateProductionOrder(id, data);
  await logMutation("update", "Продакшн", id, data.orderCode || data.project || id);
  revalidatePath("/production");
  revalidatePath("/works");
  revalidatePath("/history");
}
...
export async function deleteProductionOrderAction(formData: FormData) {
  await requireEdit();
  const { deleteProductionOrder } = await import("@/modules/production/service");
  const id = String(formData.get("id") ?? "");
  await deleteProductionOrder(id);
  await logMutation("delete", "Продакшн", id, id);
  revalidatePath("/production");
  revalidatePath("/works");
  revalidatePath("/history");
}
...
export async function createDesignOrderAction(formData: FormData) {
  await requireEdit();
  const { createDesignOrder } = await import("@/modules/design/service");
  const data = parseDesignForm(formData);
  const created = await createDesignOrder(data);
  await logMutation(
    "create",
    "Дизайн",
    created.id,
...
export async function updateDesignOrderAction(formData: FormData) {
  await requireEdit();
  const { updateDesignOrder } = await import("@/modules/design/service");
  const id = String(formData.get("id") ?? "");
  const data = parseDesignForm(formData);
  await updateDesignOrder(id, data);
  await logMutation("update", "Дизайн", id, data.orderCode || data.project || id);
  revalidatePath("/design");
  revalidatePath("/works");
  revalidatePath("/history");
}

export async function deleteDesignOrderAction(formData: FormData) {
  await requireEdit();
  const { deleteDesignOrder } = await import("@/modules/design/service");
  const id = String(formData.get("id") ?? "");
  await deleteDesignOrder(id);
  await logMutation("delete", "Дизайн", id, id);
  revalidatePath("/design");
  revalidatePath("/works");
  revalidatePath("/history");
}
